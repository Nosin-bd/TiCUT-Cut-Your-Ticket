...
import { Heading,Button,ScrollView, View, Stack, Box,Text, HStack, Spinner, Badge} from 'native-base';
...
export default function MyTripScreen() {
    ...
    const { trips, setTrips } = useContext(BookingContext);
  ...
  useEffect(() => {
    ...
    async function fetchBookings() {
      const data = []
      const querySnapshot = await firestore().collection("bookings").orderBy('date_time', 'desc').where('user_id', '==', user.uid).get();
  ...
  const handlePrintTicket = async(trip) => {
    let seatsName = '';
            ...
            <div class="ticket" aria-hidden="true">
              <div class="ticket__header">
                <div class="ticket__co">
                  ...
                  <span class="ticket__co-name">TICUT</span>
                  <span class="u-upper ticket__co-subname">Student based bus ticket booking application.</span>
                </div>
              </div>
              <div class="ticket__body">
                <p class="ticket__route">${trip.bus_name}</p>
                <p class="ticket__description">${trip.route.from }- ${trip.route.to}</p>
                <div class="ticket__timing">
                  <p>
                    <span class="u-upper ticket__small-label">Date and Time</span>
                    <span class="ticket__detail">${trip.trip_date} (${trip.trip_time})</span>
                  </p>
                  <p>
                    <span class="u-upper ticket__small-label">Seat(s)</span>
                    <span class="ticket__detail">${seatsName}</span>
                  </p>
                  <p>
                    <span class="u-upper ticket__small-label">Fares</span>
                    <span class="ticket__detail">${trip.fares}</span>
                  </p>
                </div>
                <p class="ticket__fine-print">This ticket cannot be transferred to another.</p>
                <p class="u-upper ticket__admit">Have a nice journey!</p>
              </div>
            </div>
          </div>
        </body>
    ...
    let filePath = RNFetchBlob.fs.dirs.DownloadDir + '/' + trip.id + '.pdf';
    
    RNFetchBlob.fs.writeFile(filePath, file.base64, 'base64')
        .then(response => {
          toast.show({description: "pdf downloaded successfully!"});
        })
        .catch(errors => {
    ...
    <SafeAreaView>
        ...
        <Stack px={6} pb={6} alignContent={'center'}>
          ...
          
            {
              trips.length > 0 && (
                <Stack my={4} w={'100%'}>
                  <Heading my={3} color={'theme.600'} size="sm">My Trips:</Heading>
                  {
                      ...
                      return (
                        <Box key={index} mb={2} alignItems="center">
                            <Box w={'100%'} rounded="lg" overflow="hidden" borderColor="coolGray.200" borderWidth="1" _dark={{
                                ...
                                <Stack p={3} space={3}>
                                  <Stack space={2}>
                                    <Heading color={'#000000'} size="xs">{trip.bus_name}</Heading>
                                    <Badge bg={'theme.second'}><Text color={'theme.color'}>{`${trip.route.from }- ${trip.route.to}`}</Text></Badge>
                                        <HStack flexWrap={'wrap'}>
                                          <Badge w={'60%'} bg="theme.third">{`Seat(s): ${seatsName}`}</Badge>
                                          <Badge w={'40%'} bg="theme.third">{`Fares: ${trip.fares} Tk.`}</Badge>
                                          <Badge w={'100%'} bg="theme.third">{`Departure time : ${trip.trip_date} (${trip.trip_time})`}</Badge>
                                          <Badge w={'100%'} bg={'theme.third'}>{`Booked date: ${trip.booked_date}(${trip.booked_time})`}</Badge>
                                          <Badge colorScheme="coolGray">{`Status: ${trip.status == 1 ? 'Processing' : 'Confirmed'}`}</Badge>
                                        </HStack>
                                    <HStack justifyContent={'flex-end'}>
                                      <Button size={'xs'} onPress={() => {handlePrintTicket(trip)}} bg={'theme.button'} ml={3}>Download Ticket</Button>
                                    </HStack>
